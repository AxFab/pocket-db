...
describe("Collection dropIndex", () => {
  it("drops an existing index and removes it from the index list", () => {
    ...
    assert.deepEqual(users.indexes, [{ field: "age", type: "number", unique: false }]);
  ...
  it("replays dropIndex and rebuilds the remaining indexes correctly on reopen", () => {
    ...
    assert.deepEqual(loaded.indexes, [{ field: "age", type: "number", unique: false }]);
    assert.deepEqual(
  ...
  it("allows recreating an index after it has been dropped", () => {
    ...
    assert.deepEqual(result, { acknowledged: true, field: "role", type: "string", unique: false });
